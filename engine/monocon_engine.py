...
class MonoconEngine(BaseEngine):
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        def save_kitti_format(output_path, sample_idx, det):
            formatted_idx = str(sample_idx).zfill(6)
            filename = f"{output_path}/data/{formatted_idx}.txt"
            with open(filename, 'w') as f:
                for i in range(len(det['name'])):
                    line = f"{det['name'][i]} 0.0 {det['occluded'][i]} {det['alpha'][i]} " \
                        f"{det['bbox'][i][0]} {det['bbox'][i][1]} {det['bbox'][i][2]} {det['bbox'][i][3]} " \
                        f"{det['dimensions'][i][0]} {det['dimensions'][i][1]} {det['dimensions'][i][2]} " \
                        f"{det['location'][i][0]} {det['location'][i][1]} {det['location'][i][2]} " \
                        f"{det['rotation_y'][i]} {det['score'][i]}\n"
                    f.write(line)



        # Saving in KITTI format
        output_path = 'result'
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        
        for det in eval_container['img_bbox']:
            if det['sample_idx'].size > 0:  # Check if sample_idx is not empty
                save_kitti_format(output_path, det['sample_idx'][0], det)
